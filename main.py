...
# ================================
# আপনার hf_uploader.py ফাইলটি বটের সাথে একই ডিরেক্টরিতে রাখুন
...
# ================================
# স্টক ডাটা ফেচার (HFStreamingHandler ব্যবহার করে)
# ================================
class StockDataFetcher:
    ...
    async def get_all_symbols(self):
        ...
        try:
            ...
            # hf_uploader থেকে get_symbol_list ফাংশন ব্যবহার করুন
            symbols = get_symbol_list(token=HF_TOKEN)
            if symbols:
                self.all_symbols = symbols
                logger.info(f"মোট {len(self.all_symbols)}টি সিম্বল পাওয়া গেছে")
                logger.info(f"প্রথম ২০টি সিম্বল: {self.all_symbols[:20]}")
            else:
                # ব্যাকআপ: handler এর মেথড ব্যবহার
                self.all_symbols = self.handler.get_symbol_list(max_symbols=500)
    ...
    async def get_stock_data(self, symbol: str, rows=400):
        ...
        try:
            ...
            # hf_uploader এর get_symbol_latest_data ফাংশন ব্যবহার করুন
            df = get_symbol_latest_data(symbol, rows=rows, token=HF_TOKEN)
            
            if df is None or df.empty:
                # সিমিলার সিম্বল খুঁজুন
            ...
            date_col = self.handler._find_date_column(df)
            if date_col:
...
বিশ্লেষণে ব্যবহৃত রেকর্ড: {len(df)}টি সর্বশেষ রেকর্ড
{data_summary}
...
【রিভার্সাল প্যাটার্নস】
  → ডাবল টপ/বটম (M/W Pattern)
  → হেড অ্যান্ড শোল্ডার্স
  → রাউন্ডিং টপ/বটম

【কন্টিনিউয়েশন প্যাটার্নস】
  → বুলিশ/বেয়ারিশ ফ্ল্যাগ
  → পেন্যান্ট
...
【ভলিউম-প্রাইস রিলেশনশিপ】
  ...
  → প্রাইস আপ + ভলিউম ডাউন = উইক বুলিশ
  → প্রাইস ডাউন + ভলিউম আপ = স্ট্রং বেয়ারিশ
  → প্রাইস ডাউন + ভলিউম ডাউন = উইক বেয়ারিশ

═══════════════════════════════════════════════════════════
💼 সেকশন ৮: ট্রেডিং প্ল্যান
═══════════════════════════════════════════════════════════
...
【অ্যাকশন রিকমেন্ডেশন】
  → 🟢 STRONG BUY: ৮০%+ বুলিশ কনফ্লুয়েন্স
  → 🟡 BUY: ৬০-৭৯% বুলিশ কনফ্লুয়েন্স
  → ⚪ HOLD: ৪০-৫৯% ব্যালেন্স
  → 🟠 SELL: ৬০-৭৯% বেয়ারিশ কনফ্লুয়েন্স
  → 🔴 STRONG SELL: ৮০%+ বেয়ারিশ কনফ্লুয়েন্স

【কি ইনসাইটস (Top 5)】
১. [ডাটা অনুযায়ী নির্ধারণ করুন]
২. [ডাটা অনুযায়ী নির্ধারণ করুন]
৩. [ডাটা অনুযায়ী নির্ধারণ করুন]
৪. [ডাটা অনুযায়ী নির্ধারণ করুন]
৫. [ডাটা অনুযায়ী নির্ধারণ করুন]
...
💰 **SMC জোন:** [ডাটা অনুযায়ী নির্ধারণ করুন]
🕯️ **প্রাইস অ্যাকশন:** [ডাটা অনুযায়ী নির্ধারণ করুন]
📐 **কি লেভেলস:** S1/S2/S3 | R1/R2/R3
🎯 **ট্রেড প্ল্যান:** এন্ট্রি: [প্রাইস] | SL: [প্রাইস] | TP: [প্রাইস] | R:R
📊 **কনফিডেন্স:** [০-১০০]% | **অ্যাকশন:** [BUY/SELL/HOLD]
...
💡 **নোট:** এই বিশ্লেষণ সম্পূর্ণ টেকনিক্যাল ডাটা এবং প্যাটার্ন রিকগনিশনের উপর ভিত্তি করে। সবসময় নিজস্ব রিসার্চ এবং রিস্ক ম্যানেজমেন্ট প্রয়োগ করুন।
...
বট ডাটা + প্রম্পট একসাথে ফাইল তৈরি করবে
...
"এই ডাটা এবং প্রম্পট অনুযায়ী সম্পূর্ণ টেকনিক্যাল অ্যানালাইসিস করুন"
...
📄 **ফাইলে যা থাকছে:**
✅ সম্পূর্ণ ডাটা ({len(df)}টি রো)
✅ সম্পূর্ণ টেকনিক্যাল অ্যানালাইসিস প্রম্পট
